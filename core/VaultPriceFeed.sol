...
contract VaultPriceFeed is IVaultPriceFeed {
    ...
    bool public isChainlinkPriceFeed = true; //false =>witnet
    ...
    address public witnetPriceRouter; // kava 0xD39D4d972C7E166856c4eb29E54D3548B4597F53
    ...
    function setIsChainlinkPriceFeed(bool _isChainlinkPriceFeed) external override onlyGov {
        isChainlinkPriceFeed = _isChainlinkPriceFeed;
    }
    ...
    function setWitnetRouter(address _router) external override onlyGov {
    ...
    function getLatestPrimaryPrice(address _token) public override view returns (uint256) {
        if(!isChainlinkPriceFeed){
            ...
            ( int256 lastPrice,,) = IWitnetPriceRouter(witnetPriceRouter).valueFor(priceFeedKeys[_token]);
            uint256 _priceDecimals = priceDecimals[_token];  //for witnet priceDecimals must be 6

            return uint256(lastPrice).mul(PRICE_PRECISION).div(10**_priceDecimals);
        }
