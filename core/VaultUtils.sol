...
contract VaultUtils is IVaultUtils, Governable {
    ...
    function getBuyUsdkFeeBasisPoints(address _token, uint256 _usdkAmount) public view override returns (uint256) {
        return getFeeBasisPoints(_token, _usdkAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), true);
    }

    function getSellUsdkFeeBasisPoints(address _token, uint256 _usdkAmount) public view override returns (uint256) {
        return getFeeBasisPoints(_token, _usdkAmount, vault.mintBurnFeeBasisPoints(), vault.taxBasisPoints(), false);
    }
    ...
    function getSwapFeeBasisPoints(
        ...
        uint256 feesBasisPoints0 = getFeeBasisPoints(_tokenIn, _usdkAmount, baseBps, taxBps, true);
        uint256 feesBasisPoints1 = getFeeBasisPoints(_tokenOut, _usdkAmount, baseBps, taxBps, false);
    ...
    function getFeeBasisPoints(
        ...
        uint256 initialAmount = vault.usdkAmounts(_token);
        uint256 nextAmount = initialAmount.add(_usdkDelta);
        if (!_increment) {
            nextAmount = _usdkDelta > initialAmount ? 0 : initialAmount.sub(_usdkDelta);
        }

        uint256 targetAmount = vault.getTargetUsdkAmount(_token);
        if (targetAmount == 0) {
            return _feeBasisPoints;
        }
