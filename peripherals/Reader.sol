...
contract Reader is Governable {
    ...
    function getMaxAmountIn(
        ...
        uint256 maxUsdkAmount = _vault.maxUsdkAmounts(_tokenIn);

        if (maxUsdkAmount != 0) {
            if (maxUsdkAmount < _vault.usdkAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdkAmount.sub(_vault.usdkAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10**tokenInDecimals).div(10**USDK_DECIMALS);
    ...
    function getAmountOut(
        ...
        {
            uint256 usdkAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            usdkAmount = usdkAmount.mul(10**USDK_DECIMALS).div(10**tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, usdkAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, usdkAmount, baseBps, taxBps, false);
    ...
    function getVaultTokenInfo(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 2] = vault.usdkAmounts(token);
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _usdkAmount);
    ...
    function getFullVaultTokenInfo(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxUsdkAmounts(token);
